...
> **Subagent Execution**: To prevent context pollution in the main agent's window, EVERY agent execution step in this workflow (e.g., @pm, @designer, @architect, @engineer, @secdevops, @qa, @pentester) MUST be delegated to a subagent. The main agent should spawn a subagent with the specific context and task, wait for it to return the result, and only keep the final summary/artifacts in its main context.
...
6. **@qa** executes `skills/audit_code.md` (Functional checks & Edge cases).
    ...
    - *Frontend Visual Testing*: Whenever a frontend functionality is implemented or modified, @qa MUST use the `browser_subagent` tool to open the browser and perform interactive visual and functional tests on the UI, in addition to the standard programmed tests.
